...
describe('Carousel component', () => {
  const mockImages: ImageCarousel[] = Array.from({ length: 10 }).map((_, i) => ({
    id: i,
  ...
  it('Should render button link', () => {
    const { getAllByText, queryAllByTestId } = render(<Carousel images={mockImages} />);
  ...
  it('Should call embla scrollNext and scrollPrev when buttons are clicked', () => {
    render(<Carousel images={mockImages} />);

    const nextButton = screen.getByTestId('button-carousel-next');
    fireEvent.click(nextButton);
    expect(mockScrollNext).toHaveBeenCalledTimes(1);

    const prevButton = screen.getByTestId('button-carousel-prev');
