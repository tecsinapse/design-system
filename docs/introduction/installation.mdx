...
It will get the version of `cortex-react` _core_ dependency, to avoid dependency collision.
...
For icons, use [`react-icons`](https://react-icons.github.io/react-icons/icons/lia/), included as `cortex-react` dependency. To include explicitly in `package.json`:
